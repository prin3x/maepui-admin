...
const AllTabsHomePage3 = forwardRef(({ activeTab, values, setFieldValue }, ref) => {
    ...
    const { data: productData, isLoading: productLoader, refetch } = useQuery([product], () => request({
        ...
        refetchOnWindowFocus: false, select: (res) => res?.data?.data.map((elem) => { return { id: elem.id, name: elem.name, image: elem?.thumbnail?.url || placeHolderImage, slug: elem?.slug } })
    });
    const { data: categoryData, isLoading: categoryLoader } = useQuery([Category], () => request({ url: Category, params: { status: 1, type: 'product' } }), {
        refetchOnWindowFocus: false, select: (res) => res?.data?.data.map((elem) => { return { id: elem.id, name: elem.name, image: elem?.category_icon?.url || placeHolderImage, slug: elem?.slug } })
    });
