...
const ThemeOptionSubmit = (values) => {
    ...
    if (values['serviceImage1']) {
        values['options']['seller']['services']['service_1']['image_url'] = values['serviceImage1'].url
    } else values['options']['seller']['services']['service_1']['image_url'] = ''

    if (values['serviceImage2']) {
        values['options']['seller']['services']['service_2']['image_url'] = values['serviceImage2'].url
    } else values['options']['seller']['services']['service_2']['image_url'] = ''

    if (values['serviceImage3']) {
        values['options']['seller']['services']['service_3']['image_url'] = values['serviceImage3'].url
    } else values['options']['seller']['services']['service_3']['image_url'] = ''

    if (values['serviceImage4']) {
        values['options']['seller']['services']['service_4']['image_url'] = values['serviceImage4'].url
    } else values['options']['seller']['services']['service_4']['image_url'] = ''

    if (values['aboutSellerImage']) {
        values['options']['seller']['about']['image_url'] = values['aboutSellerImage'].url
    } else values['options']['seller']['about']['image_url'] = ''

    if (values['contactUsImage']) {
        values['options']['contact_us']['imageUrl'] = values['contactUsImage'].url
    } else values['options']['contact_us']['imageUrl'] = ''

    if (values['collection_banner_image']) {
        values['options']['collection']['collection_banner_image_url'] = values['collection_banner_image'].url
    } else values['options']['collection']['collection_banner_image_url'] = ''

    if (values['banner_image_url']) {
        values['options']['product']['banner_image_url'] = values['banner_image_url'].url
    } else values['options']['product']['banner_image_url'] = ''

    if (values['safe_checkout_image']) {
        values['options']['product']['safe_checkout_image'] = values['safe_checkout_image'].url
    } else values['options']['product']['safe_checkout_image'] = ''

    if (values['secure_checkout_image']) {
        values['options']['product']['secure_checkout_image'] = values['secure_checkout_image'].url
    } else values['options']['product']['secure_checkout_image'] = ''

    // About Us
    if (values['content_left_image_url']) {
        values['options']['about_us']['about']['content_left_image_url'] = values['content_left_image_url'].url
    } else values['options']['about_us']['about']['content_left_image_url'] = ''

    if (values['content_right_image_url']) {
        values['options']['about_us']['about']['content_right_image_url'] = values['content_right_image_url'].url
    ...
    values['options']['about_us']['about']['futures']?.forEach((elem, i) => {
        if (values[`futureIcons${i}`]) {
            values['options']['about_us']['about']['futures'][i]['icon'] = values[`futureIcons${i}`].url
    ...
    values['options']['about_us']['clients']['content']?.forEach((elem, i) => {
        if (values[`clientContentImage${i}`]) {
            values['options']['about_us']['clients']['content'][i]['icon'] = values[`clientContentImage${i}`].url
    ...
    values['options']['about_us']['team']['members']?.forEach((elem, i) => {
        if (values[`teamContentImage${i}`]) {
            values['options']['about_us']['team']['members'][i]['profile_image_url'] = values[`teamContentImage${i}`].url
    ...
    values['options']['about_us']['testimonial']['reviews']?.forEach((elem, i) => {
        if (values[`testimonialReviewImage${i}`]) {
            values['options']['about_us']['testimonial']['reviews'][i]['profile_image_url'] = values[`testimonialReviewImage${i}`].url
