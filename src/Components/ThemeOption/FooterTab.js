...
const FooterTab = ({ values, setFieldValue, errors, categoryData }) => {
  ...
  const { data: pageData, isLoading: pageLoader } = useQuery([PagesAPI], () => request({ url: PagesAPI }), { refetchOnWindowFocus: false, select: (data) => data.data.map((elem) => { return { id: elem.id, name: elem.title } }) });
