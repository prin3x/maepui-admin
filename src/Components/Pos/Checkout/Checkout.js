...
const Checkout = () => {
  ...
  // Getting Users data
  const { data: userData, refetch: userRefetch, isLoading: userLoader } = useQuery([user], () => request({ url: user, params: { role: 'consumer', status: 1, paginate: 15, search: customSearch ? customSearch : "" } }), { enabled: false, refetchOnWindowFocus: false, select: (data) => data.data });
