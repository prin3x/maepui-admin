...
const HomePage5Submit = (values) => {
    ...
    if (values['productDealsImage']) {
        values['content']['product_with_deals']['deal_of_days']['image_url'] = values['productDealsImage'].url
    } else values['content']['product_with_deals']['deal_of_days']['image_url'] = ''

    if (values['homeBannerImage']) {
        values['content']['home_banner']['main_banner']['image_url'] = values['homeBannerImage'].url
    ...
    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['image_url'] = values[`bankOfferImage${i}`].url
        } else values['content']['bank_wallet_offers']['offers'][i]['image_url'] = ""
    })

    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferBankImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['bank_image_url'] = values[`bankOfferBankImage${i}`].url
