...
const AllOrdersTable = ({ data, ...props }) => {
  ...
  const headerObj = {
    ...
    column: [
      ...
      { title: 'CustomerName', apiKey: 'customer', subKey: ['name'] },
      { title: 'TotalAmount', apiKey: 'total_amount', type: 'price' },
  ...
  let orders = useMemo(() => {
    return headerObj?.data?.map((element) => {
      ...
      element.customer_name = <span className="text-capitalize">{element?.customer?.name}</span>;
      return element;
    });
  }, [headerObj?.data]);
