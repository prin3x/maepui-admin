...
import useUploadSingleImage from '../../Utils/Hooks/CustomHooks/useUploadSingleImage';
...
const FileUploadField = ({
  ...
  const removeImage = (result) => {
    if (props.name) {
      setFieldValue(
        props?.name,
        Array.isArray(values[props.name]) ? values[props.name].filter((el) => el !== result.key) : null,
      );
      setSelectedImage(selectedImage.filter((elem) => elem.key !== result.key));
    ...
    return (
      <>
        {selectedImage?.length > 0 &&
          selectedImage?.map((result) => (
            <li key={result.key}>
              <Image src={result?.url || result} className="img-fluid" width={100} height={100} alt="" priority />
              <p>
