...
const ShowTable = ({
  ...
  const orignalDataLength = headerData?.data?.data?.filter((elem) => elem.system_reserve == '1').length;
  const updateStatus = useMutation({
    mutationFn: (value) => {
      return request({ url: `${url}/${value.id}/status`, method: 'PATCH', data: { status: value.status } }, router);
    },
    onSuccess: () => {
  ...
  const deleteItem = async (id) => {
    try {
      const response = await request({ url: `${url}/${id}`, method: 'DELETE' });
    } catch (error) {
  ...
  const {
    ...
    onSuccess: () => {
      // Assuming refetch is a function to refresh the data after deletion
      refetch();
    },
  });
              ...
              >
                ...
                {elem.sorting ? (
                  <div className="filter-arrow">
                    {sortBy?.field == elem.apiKey && sortBy.sort == 'desc' ? <RiArrowUpSFill /> : <RiArrowDownSFill />}
                  </div>
                ) : (
                  ''
                )}
                ...
                {headerData?.column.map((item, i) => (
                  ...
                  >
                    {item.type == 'date' ? (
                      ...
                      <Avatar
                        data={getSubKeysData(tableData[item?.apiKey], item?.subKey)}
