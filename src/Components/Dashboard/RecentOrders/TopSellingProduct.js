  ...
  return (
    ...
    >
      <div className="top-selling-table datatable-wrapper table-responsive">
        ...
        <Table>
          <tbody>
            {data?.data?.slice(0, 5)?.map((elem, i) => (
              <tr key={i}>
                <td>
                  <div className="img-info">
                    <Avatar data={elem?.thumbnail?.url} placeHolder={placeHolderImage} name={elem} />
                    <div>
                ...
                <td>
                  <h6>{'orders'}</h6>
                  <h5>{elem?.orders_count || 0}</h5>
                </td>
                <td>
                  <h6>{'stock'}</h6>
                  <h5>{elem?.stock_quantity}</h5>
                </td>
                <td>
                  <h6>{'amount'}</h6>
                  <h5>{elem?.order_amount?.toFixed(2) || 0}</h5>
                </td>
              </tr>
            ))}
            {!data?.length && (
